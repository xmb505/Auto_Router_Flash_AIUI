#!/usr/bin/env python3
# newifid2/6.openwrt_sysupgrade.py — scp 上传 sysupgrade 固件到 OpenWrt /tmp
#                                     + SSH sysupgrade -n 烧入持久 rootfs
#
# 适用机型: Newifi D2 (新路由3) / Lecoo — MT7621A
# 前置:    5.breed_flash_firmware.py 已完成，路由器已启动到 initramfs OpenWrt (192.168.1.1)
#          用户已有 squashfs-sysupgrade.bin（放在 files/ 下）
# 后置:    sysupgrade -n 执行 → 路由器自动重启进正式 OpenWrt（有持久 rootfs）
# 来源:    cr660x/7.firmware_upload_on_openwrt.py（复用适配）
#
...
def ssh_sysupgrade(ip: str, ssh_pwd: str, remote_path: str, timeout: int) -> None:
    """SSH 进路由器跑 sysupgrade -n，连接被远端关闭视为成功。"""
    ...
    # sysupgrade -n 成功后路由器 reboot → 远端关闭连接 → ssh exit ≠ 0 (246)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Newifi D2 步骤 6：scp 上传 sysupgrade 固件 + SSH sysupgrade -n 烧入持久 rootfs（路由器自动重启）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python3 6.openwrt_sysupgrade.py --file files/immortalwrt-squashfs-sysupgrade.bin\n"
            "  python3 6.openwrt_sysupgrade.py --file files/immortalwrt-squashfs-sysupgrade.bin --debug\n"
        ),
    )
...
def help_json() -> None:
    schema = {
        "script": STEP_NAME,
        "description": "Newifi D2 步骤 6：scp 上传 sysupgrade 固件 + sysupgrade -n 烧入持久 rootfs",
        "args": [
        ...
        "stdin_contract": {
            "expects": None,
            "produces": "含 local_file/remote_file/action=upload+sysupgrade 的成功 JSON",
        },
    }
