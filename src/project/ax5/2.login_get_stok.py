#!/usr/bin/env python3
# AX5 步骤 2：登录并获取 stok token
#
# 适用机型: Redmi AX5 (RA67 / RM1800) — IPQ6000
# 加密: SHA1 only (newEncryptMode=0)
...
# AX5 已知 KEY（1.0.26 固件 init.html 可能无 JS 引用时回退用）
...
def fetch_crypto_key(ip: str, timeout: int) -> tuple:
    ...
    try:
        ...
        return KNOWN_KEY, "hardcoded fallback (AX5 known constant)"
...
def help_json() -> None:
    schema = {
        ...
        "description": "AX5 步骤 2：登录并获取 stok（KEY 运行时探测或回退已知常量）",
        "args": [
