...
# MTD 布局: mtd18=rootfs, mtd19=rootfs_1（各 36MB）
#
# ⚠️ 前提: .ubi/.bin 文件**已经**通过 5.firmware_upload_on_miwifi.py 上传到 /tmp/
...
# 使用:
#   # 1) 上传文件
#   ./5.firmware_upload_on_miwifi.py --file files/openwrt.ubi
#   # 2) 烧到指定 mtd (对侧烧更安全)
...
# AX5 MTD 映射（mtd18=rootfs, mtd19=rootfs_1）
# 来源: old_coding/Auto_Flash_Router/AX5/doc/check_init_logic.md
MTD_PARTITIONS = {
    0: "mtd18",   # rootfs
    1: "mtd19",   # rootfs_1
}
...
# ============ 复用: 同目录工具脚本 ============
def run_tool(tool: str, args: list, debug: bool = False) -> tuple:
...
def ubiformat_mtd(file_name: str, part: int, ip: str, ssh_pwd: str,
    ...
    mtd_dev = f"/dev/{MTD_PARTITIONS[part]}"
...
def help_json() -> None:
    schema = {
        "script": "miwifi_2_openwrt",
        "description": "AX5: 烧镜像到指定 mtd (文件已通过 5.firmware_upload_on_miwifi.py 上传到 /tmp/)",
        "args": [
            ...
            {"name": "--part", "type": "int", "default": None,
             "required": False, "description": "烧到 mtd18 (part=0) 或 mtd19 (part=1)。不传时自动选对侧"},
            {"name": "--file-name", "type": "string", "default": None,
             "required": True, "description": "/tmp/ 下的文件名"},
            {"name": "--debug", "type": "flag", "default": False,
             "required": False, "description": "详细日志"},
            {"name": "--probe-only", "type": "flag", "default": False,
             "required": False, "description": "只探测当前 mtd, 不真烧"},
            {"name": "--yes", "type": "flag", "default": False,
             "required": False, "description": "跳过 confirm"},
            {"name": "--timeout", "type": "int", "default": DEFAULT_TIMEOUT,
             "required": False, "description": "网络超时秒"},
        ],
        "examples": [
            "./5.firmware_upload_on_miwifi.py --file files/libwrt-...-factory.ubi",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--part", type=int, choices=[0, 1], default=None,
                   help="烧到 mtd18 (part=0) 或 mtd19 (part=1)。不传时自动探测当前活跃 mtd 并选对侧")
    p.add_argument("--file-name", required=True,
                   help="/tmp/ 下的文件名 basename, 例如 xxx.ubi (5.firmware_upload_on_miwifi.py 已上传). 不要传完整路径! 传 files/xxx.ubi 会找不到.")
...
def main() -> int:
    ...
    if err:
        emit_err(f"探测当前 mtd 失败: {err}", reason="ssh_failed")
        return 1

    # 自动选对侧: 不传 --part 时, 当前 mtd18 → target mtd19, 反之亦然
    if args.part is None:
        args.part = 1 if current_mtd == "mtd18" else 0
        log(f"--part 未传, 自动选中对侧: part={args.part}", debug=debug)

    target_mtd = MTD_PARTITIONS[args.part]
    ...
    if not writing_to_inactive:
        ...
        if not args.yes and not args.probe_only:
            emit_err(safety_warning + " (加 --yes 强制继续, 或用 --probe-only 只探测)",
                     reason="mtd_write_failed", recoverable=True)
            return 1
    ...
    if not ok:
        emit_err(f"文件不存在或不可访问: {ls_out}", reason="file_not_found")
        return 1
    ...
    if not ok:
        emit_err(f"ubiformat 失败: {ubi_out}", reason="mtd_write_failed",
                 recoverable=False)
        return 1
    ...
    emit_ok({
        ...
        "next_step": (
            "1) 可选切启动分区: ./set_miwifi_uboot_partition.sh --part {0|1}\n"
