#!/usr/bin/env python3
# AX3600 步骤 4：通过官方 API 上传并刷写固件（升级/降级通用）
#
...
# 来源:    AX6 4.official_upgrade.py 剥壳（API 链路 100% 相同）
#
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="AX3600 步骤 4：通过官方 API 上传并刷写固件（升级/降级通用，清 NVRAM）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python3 4.official_upgrade.py --stok <token> --file files/R3600_1.0.17.bin\n"
            "  python3 2.login_get_stok.py | python3 4.official_upgrade.py --file files/R3600_1.0.17.bin\n"
        ),
    )
