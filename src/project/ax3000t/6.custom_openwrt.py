#!/usr/bin/env python3
# AX3000T 步骤 6：应用自定义 OpenWrt overlay (主题/配置等)
#
# 适用机型: 小米路由器 AX3000T (RD03) — 已刷 ImmortalWrt/OpenWrt
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="AX3000T 步骤 6：应用自定义 OpenWrt overlay (主题/配置等) 到路由器, 自动重启",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python3 ./6.custom_openwrt.py --file files/overlay-new.tar.gz\n"
            "  python3 ./6.custom_openwrt.py --ip 192.168.1.1 --file files/overlay-new.tar.gz\n"
            "  python3 ./6.custom_openwrt.py --file files/overlay-new.tar.gz --ssh-pwd mypass\n"
        ),
    )
...
def main() -> int:
    ...
    # tar.gz 内部应直接以 upper/ 为顶层目录（OpenWrt overlay 惯例）
    extract_cmd = (
        f"cd /tmp && tar -xzf {DEFAULT_REMOTE_NAME} "
        f"&& if [ -d upper ]; then cp -a upper/* /overlay/upper/; "
        f"elif [ -d overlay ]; then cp -a overlay/* /overlay/upper/; "
        f"else echo 'UNSUPPORTED_LAYOUT' && exit 1; fi "
        f"&& rm -rf upper overlay {DEFAULT_REMOTE_NAME} "
        f"&& echo OK"
    )
    ...
    if not ok:
        emit_err(f"解压失败: {out}")
        return 1
    if "UNSUPPORTED_LAYOUT" in out:
        emit_err("tar.gz 内部结构不支持：需要 upper/ 或 overlay/ 为顶层目录")
        return 1
    if "OK" not in out:
