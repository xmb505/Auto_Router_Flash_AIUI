# AX6 官方 API 刷固件 — `4.official_upgrade.py`
...
| 项 | 值 |
|----|----|
| 适用 | AX6 (RA69) — IPQ8071A，工厂态和已初始化都能用 |
...
| 路由器 | 起始 FW | 目标 FW | 方向 | 验证日期 | 验证人 | 结果 |
|--------|--------|---------|------|---------|-------|------|
| A0U842226 (Redmi_0BA0_F45F) | 1.1.10 (工厂) | 1.1.3 | upgrade | 2026-06-08 | 用户 | ✅ 29623224 bytes，romversion 切到 1.1.3 |
| A0U842226 (Redmi_0BA0_F45F) | 1.1.3 | 1.0.16 | downgrade | 2026-06-08 | 用户 | ✅ 29361084 bytes，romversion 切到 1.0.16 |

> 两次刷写之间通过 `1.official_init.py` 重新初始化（`recovery=1` 清完 NVRAM 后 `inited=0`），
> 整条链路 `2.login_get_stok.py → 4.official_upgrade.py` 在两台不同固件版本上行为一致。

### 验证标记

| 验证项 | 标记 |
|--------|------|
| `downgrade=1` 不影响升级流 | ✅ 实测 1.1.10 → 1.1.3 |
| `recovery=1` 真清 NVRAM | ✅ 两次刷完都 `inited=0` |
| 4 步 API 链都返 `code:0` | ✅ 两次完整跑通 |
| 1.1.3 → 1.0.16 跨版本降级 | ✅ 实测 |
| 1.0.16 → 1.1.10 跨版本升级 | ⚠️ 未实测 |
| 1.0.x 系列内部升降 | ⚠️ 未实测 |
| 非 1.x 系列固件 | ❌ 未实测（AX6 没这固件）|
...
| `刷机许可被拒 (code=...)` | 固件与硬件不匹配（型号 / region 错）| 换对应硬件版本的固件 |
| `flash_rom 被拒` | 同上 / NVRAM 锁 | 路由器断电 30s 后重试 |

## 与 `old_coding/.../downgrade.py` 的关系

新脚本是 `downgrade.py` 的精简+重构版本：
...
- `old_coding/Auto_Flash_Router/AX6/downgrade.py` — 原始 4 步链路实现
- `old_coding/Auto_Flash_Router/AX5/doc/check_init_logic.md:181` — 同链路在 AX5 上的 API 表格
- `old_coding/router-flash/ax6/downgrade.py` — 同一份实现的 router-flash 融合版
