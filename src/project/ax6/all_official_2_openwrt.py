#!/usr/bin/env python3
# all_official_2_openwrt.py — AX6 官方固件 → OpenWrt/LibWrt 全自动刷机
#
...
def build_argparse() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="AX6 官方固件 → OpenWrt/LibWrt 全自动刷机",
...
def help_json_schema() -> None:
    schema = {
        ...
        "config_format": "INI: [firmware].ubi_file, [firmware].overlay_file (可选)",
        "examples": [
            "python3 all_official_2_openwrt.py",
            "python3 all_official_2_openwrt.py --pwd 12345678",
            "python3 all_official_2_openwrt.py --config my.ini --debug",
...
def main() -> int:
    ...
    if overlay:
        overlay = os.path.join(SCRIPT_DIR, overlay) if not os.path.isabs(overlay) else overlay
    ...
    try:
        ...
        if "ra69" not in model.lower() and "ax6" not in model.lower():
            raise RuntimeError(
                f"硬件不匹配: 期望 AX6/RA69, 实际 model={model}")
        log("硬件验证通过: AX6")
        steps_done.append("verified_ax6")
        ...
        fname = os.path.basename(target)  # .sh 返回 /tmp/xxx, 取 basename
