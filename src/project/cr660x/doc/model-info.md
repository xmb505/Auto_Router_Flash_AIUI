# CR660X (CR6606 / CR6608 / CR6609 / TR606 / TR608 / TR609) — 机型信息
...
|------|------|--------|------|
| `xiaomi.router.cr6606` | CR6606 / TR606 | 中国联通 | ✅ 2026-06-11 |
| `xiaomi.router.cr6608` | CR6608 / TR608 | 中国移动 | ✅ 2026-06-11 |
| `xiaomi.router.cr6609` | CR6609 / TR609 | 中国电信 | ✅ 2026-06-16 (IP 192.168.31.1) |

## 冷知识：刷 OpenWrt 后都叫 CR6608

CR6606/TR606、CR6608/TR608、CR6609/TR609 硬件完全相同（MT7621A），只是运营商定制固件不同。
刷入 OpenWrt/ImmortalWrt 后，所有型号都报告为 `CR6608`——因为 OpenWrt 目标设备的 DTS 只区分了这一个型号名。所以 initramfs 阶段和正式 OpenWrt 中看到的硬件信息都是 CR6608，不影响使用。
...
| 默认 IP (移动/TR608) | `192.168.31.1` 或 `192.168.2.1` |
| 默认 IP (电信/TR609) | `192.168.31.1` 或 `192.168.2.1` |
| 默认密码 (工厂态) | `admin` |

## SSH root 密码

SSH root 密码取决于 SSH 启用方式（2026-06-16 实机验证）：

| 启用方式 | SSH root 密码 | 说明 |
|---------|--------------|------|
| extendwifi (辅助 WiFi 注入) | **管理密码** | 移动/电信版：SSH root 密码 = 路由器管理密码 |
| smartcontroller scene 注入 | **root** | 两种方式最终都设 root 权限，只是密码不同 |
