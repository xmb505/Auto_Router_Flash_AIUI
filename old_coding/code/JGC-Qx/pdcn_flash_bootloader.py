...
def flash_bootloader(ip_address):
    """
    刷入bootloader到路由器并返回执行结果
    """
    try:
        # 构造URL
        ...
        payload = {
            ...
            'SystemCmd': 'mtd_write write /tmp/pb-boot.img Bootloader'
        }
        ...
        print(f"刷入bootloader时出错: {e}", file=sys.stderr)
        return None
...
def main():
    ...
    # 刷入bootloader并获取结果
    result = flash_bootloader(ip_address)
