...
def put_bootloader(ip_address):
    """
    上传bootloader到路由器并返回执行结果
    """
    try:
        # 构造URL
        ...
        payload = {
            ...
            'SystemCmd': 'wget -O /tmp/pb-boot.img http://192.168.123.2:8080/chfs/shared/pb-boot.img'
        }
        ...
        print(f"上传bootloader时出错: {e}", file=sys.stderr)
        return None
...
def main():
    ...
    # 上传bootloader并获取结果
    result = put_bootloader(ip_address)
