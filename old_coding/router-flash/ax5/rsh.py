#!/usr/bin/env python3
"""SSH/SCP 工具（基于 sshpass + 系统 ssh），兼容老旧加密算法"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='SSH/SCP 工具（sshpass + 系统 ssh）',
    ...
    try:
        if args.action == 'run':
            ...
            if code == 0:
                print(json.dumps({"status": "ok", "action": "put", "local": args.arg1, "remote": args.arg2}))
            sys.exit(code)
            ...
            if err: print(err, end='', file=sys.stderr)
            if code == 0:
                print(json.dumps({"status": "ok", "action": "get", "remote": args.arg1, "local": args.arg2}))
            sys.exit(code)
