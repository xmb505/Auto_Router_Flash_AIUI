...
def http_post(url: str, data: dict, timeout: int = 10) -> tuple:
    """HTTP POST 请求 (表单)"""
...
def auto_init(router_ip: str, ssid: str, wifi_pwd: str, admin_pwd: str) -> dict:
    ...
    result = http_post(set_url, post_data)
    if result.get('code') != 0:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='红米 AX5 自动初始化向导（刷机前置）',
    ...
    parser.add_argument('--ssid', default='Redmi_7D5A', help='Wi-Fi SSID (默认: Redmi_7D5A)')
